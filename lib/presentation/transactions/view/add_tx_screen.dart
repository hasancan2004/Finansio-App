// lib/presentation/transactions/view/add_tx_screen.dart
import 'dart:async';

import 'package:drift/drift.dart' show Value;
import 'package:finansio/presentation/reports/viewmodel/forecast_providers.dart';
...
// ✅ AI
import 'package:finansio/domain/engine/spending_analyzer.dart';

// ✅ learning
import 'package:finansio/core/utils/text_utils.dart';

// ✅ BİLDİRİM SERVİSİ İÇİN EKLENDİ
...
class _AddTxScreenState extends ConsumerState<AddTxScreen> {
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      // ✅ BÜTÇE KONTROLÜ VE BİLDİRİM TETİKLEME (Sadece giderlerde)
      if (amount < 0) {
        try {
          // Provider invalidate edildiği için taze bütçe verilerini çekiyoruz
          final currentBudgets = await ref.read(budgetStatusesProvider.future);

          final catList = currentBudgets.where((b) => b.category.id == cat.id).toList();
          if (catList.isNotEmpty) {
            final catBudget = catList.first;

            if (catBudget.progress != null) {
              if (catBudget.progress! > 1.0) {
                // Limit %100'ü aştıysa
                await NotificationService.showBudgetExceeded(
                  categoryName: catBudget.category.name,
                  spent: catBudget.spent,
                  limit: catBudget.budget!,
                );
              } else if (catBudget.progress! >= 0.8) {
                // Limit %80'i geçtiyse (Yaklaşma Uyarısı)
                await NotificationService.showBudgetWarning(
                  categoryName: catBudget.category.name,
                  percent: catBudget.progress! * 100,
                );
              }
            }
          }
        } catch (e) {
          debugPrint("Bütçe bildirimi tetiklenirken hata: $e");
        }
      }
