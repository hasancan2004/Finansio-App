...
import 'package:finansio/screens/add_tx_screen.dart';
// import 'package:finansio/screens/categories_screen.dart'; // Artık ProfileDashboard'dan gidilebilir
// import 'package:finansio/screens/reports_screen.dart';    // Artık ProfileDashboard'dan gidilebilir
// import 'package:finansio/screens/settings_screen.dart';   // Artık ProfileDashboard'dan gidilebilir
import 'package:finansio/widgets/empty_state.dart';
import 'package:finansio/widgets/tx_search_delegate.dart';
...
// ✅ Forecast AI (kullanmasan bile dursun)
import 'package:finansio/ai/forecast_engine.dart';

// ✅ Recurring Engine
import 'package:finansio/services/recurring_engine.dart';

// ✅ NEW: Yeni Profil Dashboard Ekranımız
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _refreshForecast() {
    // ✅ Aykırı değer ekle/sil sonrası forecast'ın anında güncellenmesi için
  ...
  void initState() {
    ...
    Future.microtask(() async {
      ...
      try {
        ...
        if (created > 0) {
          // recurring tx eklendiyse forecast da güncellensin
          _refreshForecast();
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> openAddTx() async {
      ...
      _refreshForecast(); // ✅ ekledikten sonra anında güncelle
    }
    ...
    return AppScaffold(
      ...
      actions: [
        ...
        // ✅ 3 noktalı menü uçuruldu, yerine direkt en lazım olan filtre/tarih butonları geldi
        IconButton(
        ...
        children: [
          ...
          Padding(
            ...
            child: AppHeader(
              ...
              // ✅ Artık Avatar tıklanabilir ve Dashboard'a yönlendiriyor!
              trailing: Material(
                ...
                child: InkWell(
                  ...
                  child: CircleAvatar(
                    radius: 20, // Hafif büyütüldü (18'den 20'ye)
          ...
          // FORECAST
          if (filter == FilterRange.thisMonth && range == null) ...[
            const SizedBox(height: 10),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: _ForecastCard(cs: cs),
            ),
          ],
  ...
  Widget _buildTransactionListEmbedded(
    ...
    return txs.when(
      data: (list) {
        ...
        // ✅ garantili: yeni -> eski (AMA local time ile)
        final items = [...list];
...
// ✅ Forecast kart widget’ı
class _ForecastCard extends ConsumerWidget {
  final ColorScheme cs;
  const _ForecastCard({required this.cs});
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: async.when(
          ...
          data: (f) {
            final progress = (f.dayOfMonth / f.daysInMonth).clamp(0.0, 1.0);
            final netColor = f.predictedNetMonth >= 0 ? Colors.green : Colors.red;
            ...
            return Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.auto_graph, color: cs.primary),
                    const SizedBox(width: 8),
                    Text(
                      "Ay Sonu Tahmin",
                  ...
                  children: [
                    Expanded(
                      child: _miniStat(
                        context,
                        title: "Tahmini Gider",
                        value: _formatTry(f.predictedExpenseMonth),
                        color: Colors.red,
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: _miniStat(
                        context,
                        title: "Tahmini Gelir",
                        value: _formatTry(f.predictedIncomeMonth),
                        color: Colors.green,
                      ),
                    ),
                  ],
                ...
                _miniStat(
                  context,
                  title: "Tahmini Net",
                  value: _formatTry(f.predictedNetMonth.abs()),
                  color: netColor,
                ),
